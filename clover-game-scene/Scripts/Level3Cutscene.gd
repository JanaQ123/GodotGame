...
func _ready():
	...
	backgroundCloud.get_node("StaticBody2D/CollisionPolygon2D").set_deferred("disabled", false)
...
func _on_area_2d_body_entered(body: Node2D) -> void:
		if body.is_in_group("player"):
			...
			backgroundCloud.visible = false
			backgroundCloud.get_node("StaticBody2D/CollisionPolygon2D").set_deferred("disabled", true)
